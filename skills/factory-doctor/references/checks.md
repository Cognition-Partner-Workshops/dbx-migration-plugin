...
`install-dbx-factory`, the only fix. `ok` proves the repo matches the lock's last-sync receipt,
not live org state.
...
| `playbooks_in_sync` | the lock `.migration/playbooks.lock.json` is missing (fail; `skipped` only under `--role setup`), unreadable, has a `malformed` entry (non-string `sha256`/`repo_file`/`installed_at`), or differs from the repo playbook files — `stale` sha, `missing` macro, `unknown` macro in the lock but not the repo, or a `*.md` under `playbooks/` absent from the 0-README Files table. `ok` proves repo == last sync receipt, not live state. Re-run `install-dbx-factory` | `.migration/playbooks.lock.json` |
