...
|---|---|---|
| `approx_percentile(x, 0.5)` (T-Digest; `REAL` for `DECIMAL` input **(probe)**) | exact `percentile(x, 0.5)` cast to `FLOAT`; `percentile_approx` only if a decision row accepts approximation on both sides | row 52, trap "Approximate aggregates" |
...
`median_order_total`. The recon query uses the exact percentile on **both** sides; comparing an approximate source
