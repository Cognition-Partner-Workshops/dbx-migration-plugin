...
| `SUM(decimal)/COUNT(*)` -> `decimal(31,21)` on Trino, `(38,?)` on Databricks | explicit `CAST(... AS DECIMAL(p,2))` for this money contract; any other result scale needs an explicit target cast and `identity` | row 2 |
...
**Canonicalization**: the converted SQL explicitly casts `avg_order_value` and `revenue_per_order` to
`DECIMAL(p,2)`, so `decimal_round` places 2 applies to those recomputed money columns (both engines round half
away from zero **(probe)**, so `half_up`). Any other result scale is pinned by an explicit target cast and
reconciled with `identity`; `identity` also applies to `lifetime_revenue` (stored scale).
`datetime_utc_truncate_ms` is not needed (no timestamps in the output).
