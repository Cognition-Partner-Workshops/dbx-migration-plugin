...
-- BT/ET has no drop-in: per-statement commits, idempotent triple, campaign lock row for exclusion (NOTE.md).
...
AS BEGIN
    ...
    -- ROLLBACK has no equivalent: partial work is kept, the re-run is idempotent, the INOUT budget is charged
    -- conservatively (counter increments before the status UPDATE, so an interrupted account is charged and redone).
    -- Fixed return code: no cited read of SQLCODE/SQLSTATE inside a handler. Lock released only if this call owns it.
    ...
    END IF;
    -- BT held write locks until ET, serialising callers. Seeded one-row lock table: claim, read back, SIGNAL if lost.
    -- The loser either sees the winner's id or fails its UPDATE with a write conflict; both land in the handler.
    ...
    IF (SELECT OWNER_RUN_ID FROM ${catalog}.${schema}.ARCHIVE_CAMPAIGN_LOCK
        ...
        SIGNAL SQLSTATE '75003' SET MESSAGE_TEXT = 'another SP_ARCHIVE_CLOSED_ACCOUNTS run holds the campaign lock';
    END IF;
    ...
    -- (archive, delete, mark) triple is idempotent: each TRANSACTION_ID is in the fact, the archive, or transiently both.
