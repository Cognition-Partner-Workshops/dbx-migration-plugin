...
-- invisible behind a newer one, and two runs for different dates never see each other's rows. Two runs for the
-- *same* date are the source's own semantics too (last EXEC wins); here the DELETE + INSERT for that date is the
-- unit of replacement, so a consumer must read after its own CALL returns, exactly as it read after its own EXEC.
CREATE OR REPLACE VIEW ${catalog}.${schema}.VW_AML_STRUCTURING AS
SELECT SCREENING_DATE, PATTERN_TYPE, CUSTOMER_ID, CUSTOMER_NAME, KYC_STATUS, ACCOUNT_ID, TXN_COUNT, TOTAL_AMOUNT,
       AVG_AMOUNT, LAST_TXN_DATE, SORT_ORDER
...
SELECT SCREENING_DATE, PATTERN_TYPE, CUSTOMER_ID, CUSTOMER_NAME, ACCOUNT_ID, CREDIT_DATE, CREDIT_AMOUNT, DEBIT_DATE,
       DEBIT_AMOUNT, DAYS_BETWEEN, SORT_ORDER
FROM ${catalog}.${schema}.AML_SCREENING_RESULT
WHERE RESULT_SET_NO = 2;

CREATE OR REPLACE VIEW ${catalog}.${schema}.VW_AML_NEW_CUSTOMER_INTL AS
SELECT SCREENING_DATE, PATTERN_TYPE, CUSTOMER_ID, CUSTOMER_NAME, ONBOARDING_DATE, DAYS_SINCE_ONBOARD, KYC_STATUS,
       TXN_COUNT AS INTL_TXN_COUNT, TOTAL_AMOUNT AS TOTAL_INTL_AMOUNT, SORT_ORDER
FROM ${catalog}.${schema}.AML_SCREENING_RESULT
