...
| BT/ET atomicity | multi-statement rollback | per-statement atomicity unless `BEGIN ATOMIC` (preview, `catalogManaged`) | Tier 2 conservation drift after a failed run (rows in archive and in source, example 06) | idempotent re-run design; kept partial work charged to any caller-visible budget from persisted state keyed by a per-call id (ledger), not from loop counters or time windows; `BEGIN ATOMIC` where the preview is acceptable | mapping states which units rely on rollback |
