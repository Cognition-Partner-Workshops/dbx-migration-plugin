...
| BT/ET atomicity and lock scope | multi-statement rollback; write locks to `ET` serialise callers | per-statement commits, no lock between statements | Tier 2 conservation drift after a failed run; Tier 1 shortfall when overlapping calls process the same rows | idempotent re-run, budget derived from a per-run ledger joined to committed status rows (never an in-memory counter, a timestamp window, or a repurposed lineage column), explicit exclusion (`max_concurrent_runs: 1` or a seeded lock row shipped with the DDL) (example 03) |
