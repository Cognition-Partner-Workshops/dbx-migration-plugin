...
| BT/ET atomicity and lock scope | multi-statement rollback; write locks to `ET` serialise callers | per-statement commits, no lock between statements | Tier 2 conservation drift after a failed run; Tier 1 shortfall when overlapping calls process the same rows | idempotent re-run, budget derived from committed rows stamped with the run's id (never an in-memory counter or a timestamp window), explicit exclusion (`max_concurrent_runs: 1` or a seeded lock row shipped with the DDL) (example 03) |
