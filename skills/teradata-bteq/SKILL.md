...
| 19 | `TRIM(LEADING 'x' FROM s)` / `TRIM(TRAILING ...)` | `ltrim('x', s)` / `rtrim('x', s)` | edge case | argument order is `ltrim([trimStr,] str)` (docs.databricks.com/aws/en/sql/language-manual/functions/ltrim: `ltrim('abc', 'acbabSparkSQL') = 'SparkSQL'`), the reverse of most engines; Teradata trims one character, Databricks trims any character in the set |
...
| 53 | `TRUNC(x, n)` (numeric) | `sign(x) * floor(abs(x) * pow(10, n)) / pow(10, n)`, then `cast(... as decimal(p, n))` to restore the type | edge case | truncation is toward zero for both signs; never `cast(x as decimal(p, n))` alone: a narrowing decimal cast *rounds* the dropped digits, so `TRUNC(1.29, 1) = 1.2` would come back `1.3` |
...
| Macro (multi statement / multi result set) | several `SEL` in one macro | new result table (+ run ledger) whose DDL is part of the unit, procedure writing one `RUN_ID` per `CALL` and publishing it last, one view per result set over the latest published run (the macro's single-request visibility) | "CREATE PROCEDURE"; `QUALIFY` per `best-practices.md` | example 05 |
...
| Volatile / global temporary tables | `CREATE VOLATILE TABLE ... ON COMMIT PRESERVE ROWS` | `CREATE TEMPORARY TABLE` / `CREATE TEMPORARY VIEW` | `databricks-dbsql` `SKILL.md` "Temp Tables" row; `references/materialized-views-pipes.md` "Temp Tables" | §5 row 88; a BTEQ volatile table that carries state across `.IF` branches becomes a persisted row keyed by the job run (`{{job.run_id}}` job parameter read as `:run_id` in each `sql_task`; example 03) -- never a "the open row" lookup on a shared table; any id it allocates in a *shared* control table is reserved there in the same task, not only in the unit-owned row |
...
| Shell `${VAR}` / `.SET` substitution | environment | DABs variables `${var.x}` at deploy time; job-level `parameters` at run time, read as `:name` in a `sql_task` file | `databricks-dabs` `references/bundle-structure.md` (variables); `databricks-jobs` `SKILL.md` "Job Parameters"; docs.databricks.com/aws/en/jobs/parameter-use (SQL named parameters) | example 03 uses build-time `${catalog}.${schema}` and run-time `:run_id` |
...
| BT/ET atomicity and lock scope | multi-statement rollback; write locks held to `ET` serialise overlapping callers | per-statement atomicity unless `BEGIN ATOMIC` (preview, `catalogManaged`); no lock is held between statements | Tier 2 conservation drift after a failed run (rows in archive and in source, example 06); Tier 1 shortfall when two overlapping calls each process the same rows | idempotent re-run design; kept partial work charged to any caller-visible budget from persisted state keyed by a per-call id (ledger), not from loop counters or time windows; a unit whose source relied on BT/ET to exclude a concurrent caller takes an explicit single-row lock (`UPDATE ... WHERE OWNER IS NULL`, read back, `SIGNAL` if not owner; example 06); `BEGIN ATOMIC` where the preview is acceptable | mapping states which units rely on rollback or on lock exclusion |
...
| Volatile tables carrying state across `.IF` branches | script-local table survives the whole script | temp table lives in one session/task | Tier 1 on control tables (status never `COMPLETED`/`FAILED`) | persist the state row keyed by the job run id, in a unit-owned table (example 03) | none |
...
| `05_macro_multi_resultset/` | `dml/macros/macro_aml_screening.sql` (fixture) | multi-statement macro with `DEFAULT DATE` parameters, date arithmetic, `QUALIFY`, three result sets -> result-table + run-ledger DDL, procedure publishing one run per `CALL`, three views on the latest published run per date | Tier 1 per result set (collapse, interleaved same-date runs), Tier 2 on date windows, Tier 3 on decimal thresholds |
