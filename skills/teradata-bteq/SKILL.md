...
| BTEQ `.IF ACTIVITYCOUNT = 0 THEN ...` | data-dependent branch | `SIGNAL` in the SQL block when the guard fails, downstream `run_if`; or an `IF` inside one scripting block | `databricks-jobs` `references/task-types.md`; `sql-scripting.md` "IF" | example 03 `01_staging_check.sql` |
...
| Decimal rounding | half-even (ANSI default) or half-up (`RoundHalfwayMagUp`) per DBS Control | `round` half-up, `bround` half-even | Tier 3 last-digit diffs; Tier 2 sum drift by a few units in the last place | use `bround` where the source was half-even | `decimal_round` places from the tolerance record; `mode` recorded (`half_even` here) |
...
| `decimal_round` | `DECIMAL,NUMBER->DECIMAL` | `mode: half_even`, `places: 2` | half-even vs half-up and intermediate-precision differences; `places` must equal the tolerance record's (2 for this fixture's monetary columns: `DECIMAL(15,2)`, `DECIMAL(18,2)`), `mode` matches the source's DBS Control setting (ANSI default half-even) |
...
string mapping in §4). Until they exist, PERIOD columns are compared as two ordinary timestamp columns and `TIME`
columns as strings, and the unit's recon plan says so.
...
| `03_bteq_control_flow_to_job/` | `dml/scripts/bteq_daily_load.btq` (fixture) | `.IF ERRORCODE`/`.IF ACTIVITYCOUNT`/`.GOTO`/`.LABEL`/`.QUIT n`, volatile `VT_BATCH`, `.EXPORT`, `CALL`, `EXEC` -> Lakeflow Job with `depends_on`/`run_if`, `SIGNAL` guards, control-table state | Tier 1 on control/log tables and downstream row counts, Tier 2 on batch totals, Tier 4 on the exported report |
