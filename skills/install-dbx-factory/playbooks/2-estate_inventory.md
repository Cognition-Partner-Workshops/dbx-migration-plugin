...
[estate export / repo / catalog metadata]  ->  asset census  ->  lineage DAG  ->  pipeline catalog
                                           ...
                                           ->  first-pass dependency register  ->  STOP B (only if intake did not fix pipeline order)
```
...
7. **Governance inventory**: with the dialect skill's governance-discovery entry (the system tables, e.g. Teradata `DBC.AllRightsV`, Redshift `SVV_RELATION_PRIVILEGES`/`pg_default_acl`, Oracle `DBA_TAB_PRIVS`/`DBA_ROLES`/redaction policies), extract every grant (grantee, privilege, grantable), role and role membership, service account that holds access, and masking/row-level policy on in-scope objects into a `## Governance inventory` section appended to `.migration/04_dependency_register.md` (grants that must exist on the target are dependencies), one FACT row per (object, grantee, privilege/policy) with its metadata query cited. The inventory records that a service account exists, never its credential. These are census objects: the coverage arithmetic includes them, so no grant silently vanishes between inventory and cutover.
...
10. **Write `<Estate>_inventory.md`** with all of the above, render the DAG to an embedded image, and update the ledger. **STOP B (per stop_mode; skipped if the intake fixed pipeline order)**: attach the inventory, recommend a first pipeline (smallest honest slice that exercises every workload surface, not the easiest); the recommendation is the stop's default, so in soft mode silence selects it, and any reply naming a different pipeline overrides it. Confirm its boundary and exclusions. Never choose for them.

## Specifications
- Deliverable: `<Estate>_inventory.md` with census, coverage arithmetic, pipeline catalog, shared-object map, parallelism profile, PROPOSED-unused set, and first-pass dependency entries; a `## Governance inventory` section appended to `.migration/04_dependency_register.md` with the grant/role/policy census.
- Validation: (1) coverage arithmetic closes exactly; (2) every lineage edge is FACT or INFERRED, never implicit; (3) every shared object has a proposed owner; (4) the recommended pipeline is justified against the catalog, and the user made the choice at STOP B when intake did not fix pipeline order.
...
## Forbidden Actions
- Do NOT pick the pipeline; enumerate, rank, recommend, and let the user choose at STOP B (skipped if the intake fixed pipeline order).
