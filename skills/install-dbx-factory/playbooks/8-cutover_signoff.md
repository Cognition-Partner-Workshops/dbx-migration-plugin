...
- Every writing routine's `routine_parity` status is `proven` (`skills/data-reconciliation/SKILL.md`, "Routine parity"); each `unproven` routine is listed by name as an exception in the evidence pack, and a `failed` one routes back.
