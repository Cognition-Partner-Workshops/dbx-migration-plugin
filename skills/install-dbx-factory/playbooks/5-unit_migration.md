...
- [ ] Finish the Devin Review round on your PR before reporting done: zero open actionable findings at the head you report (`review_clean: true`); a wrong finding is waived only by a human's `review_waived` row in `06_decisions.md`, never worked around.
