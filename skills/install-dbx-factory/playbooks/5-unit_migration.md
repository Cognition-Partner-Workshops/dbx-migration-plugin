...
- [ ] Idempotency is the rerun proof of `data-reconciliation` (`dbx-recon rerun-proof`, fixture `harness/fixtures/example_rerun/`): fresh target and a target pre-created in the table's previous committed shape; pass `--rerun-proof` to `run`.
