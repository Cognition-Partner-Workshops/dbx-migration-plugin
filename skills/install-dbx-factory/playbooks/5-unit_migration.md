...
- [ ] Idempotency is the rerun proof of `data-reconciliation` (`dbx-recon rerun-proof`, fixture `harness/fixtures/example_rerun/`): fresh target and a target pre-created in the table's previous committed shape (`--prior-proof`, the last committed `rerun_proof.json`); pass `--rerun-proof` with `--rerun-source` to `run`.
...
- [ ] Review happens once at wave close over all PASS PRs; you do not wait for or report review state.
...
- [ ] Open one PR with changed paths, full recon JSON, short summary, three-part body, cost, write targets, and SKILL FEEDBACK; PASS or FAIL is the first line of the PR body, and one-line `skill_feedback` plus `recon_cost` (result.json cost) go in the structured report.
