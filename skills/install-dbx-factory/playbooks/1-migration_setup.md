...
4. Initialize `04_dependency_register.md` (empty, with the D1-D10 taxonomy header). `05_progress.md` is not written by hand: it is regenerated from `waves/*.result.json` with `python3 skills/migration-fanout/progress.py .migration` at every wave close and read as the cutover-readiness view; `06_decisions.md` rows carry **provenance and blast radius** (which unit forced the decision, which PR argued it, and which already-merged units' assumptions it invalidates), because a decision added at wave 3 that silently changes a wave-1 unit is otherwise invisible.
