Playbook: Set up the engagement end to end: ingest the target state (what "migrated" means) as per-workload profiles, create the durable `.migration/` working memory, pin the reconciliation tolerances, and fire the access checklist, terminating in one STOP A per stop_mode.

## Overview
This is the single engagement-start playbook. It runs once, in two phases, and ends at the one confirmation (STOP A, per stop_mode) that authorizes all downstream work. Phase 1 makes every later session (parent, child, or one of fifty parallel fan-out children) already know what "migrated" means without re-deriving it. Phase 2 creates the durable working memory, pins the parity contract, and fires the access requests that gate everything else. Keeping the two phases in one playbook keeps the target state and the tolerances from drifting apart: they are confirmed together, as one artifact set, at one stop.
...
|---|---|---|
| **CORE** | everything | Unity Catalog layout (catalog/schema naming, environments), Delta conventions, naming standards, code language policy (SQL vs PySpark, when each), repo layout and Declarative Automation Bundle usage, CI gates, test conventions, forbidden patterns, secret and service-principal conventions |
...
| **PIPELINE** | ETL mappings / jobs | target runtime (Lakeflow Spark Declarative Pipelines vs Lakeflow Jobs tasks, per the `databricks-pipelines` decision tree; notebooks are evidence only), medallion layering policy, incremental vs full-load patterns, error/reject-row handling, restart and idempotency semantics, parameterization, logging |
...
| **LAKEBASE** | operational-track tables, constraints, triggers, transactional procedures, and the applications that call them (OLTP front door only; N/A for pure warehouse/ETL estates) | target = Lakebase Postgres Autoscaling project (`databricks postgres`, never the retired Provisioned tier), project/branch/endpoint/database naming, one branch per wave batch with TTL as the isolated namespace, `production` branch write policy (never from a migration session), Postgres 17 conventions (PL/pgSQL for procedures, sequences for identities, constraint and index parity policy, extension allowlist), application connectivity (OAuth token refresh, pooling, SSL) and the repoint contract, lakehouse sync of operational tables into Unity Catalog for the analytical track, synced tables from Delta back into Postgres where reference data is warehouse-mastered, compute sizing and scale-to-zero policy per environment |
...
- **Which surfaces are in scope**. An estate with no ML jobs does not need an ML-SCORING profile; record it as N/A rather than inventing one. LAKEBASE is in scope only when the front door recorded an operational track; when it is, the track split in `00_context.md` is the profile's scope statement, and the analytical track keeps SQL/PIPELINE.
...
```
THIS PLAYBOOK  ->  .migration/00_context.md          engagement facts, topology, contacts, glossary (customer terms -> plain meaning)
                   ...
                   .migration/03_recon_tolerances.json  the same contract in the form `dbx-recon` reads
                   .migration/04_dependency_register.md   appended by later playbooks
                   .migration/05_progress.md         generated by skills/migration-fanout/progress.py from waves/*.result.json; never hand-edited
...
2. Write `00_context.md` (with a `## Glossary` section) and `01_conventions.md` from the target state, the front-door intake, and the user's answers. Record the **interaction contract** in `00_context.md` alongside the notification contract: where stops are routed (Slack thread vs web session), and the question style (one question at a time, with concrete options where the answer set is small); settling this once here means no run renegotiates it live or loses questions to interrupting notifications. Process contract (stops, stop_mode, D1–D10, notifications, branch/merge, fan-out guards): read references/contract.md in the install-dbx-factory skill once per session; it is not restated here. Conventions must include branch naming for fan-out children (e.g. `migrate/<pipeline>/<wave>-<unit>`) and the PR-evidence contract (every unit PR carries its own recon output).
3. **Pin the tolerances in `03_recon_tolerances.md`**: one table, per data type and per workload surface, each row FACT (user-confirmed) or PROPOSED, plus the size-tier thresholds and the legacy-query concurrency cap. Every rate or threshold row also states the **population it is computed over** (e.g. a quarantine rate over the driver population only, a halt threshold over every population); a tolerance without its population is the most common source of silently-diluted checks. This file is the contract every recon run cites; a recon result is meaningless without it. The file also states the **recon mode**: LIVE (federation/dual-run) or DEGRADED (snapshot/sample), and includes the **amendment procedure**: a tolerance changes only by explicit user approval, recorded as a new dated version of the record with the old row preserved, plus a stated re-verification scope for already-merged waves under the old tolerance. Children never see an ambiguous tolerance; they see the current version and its date. Alongside the record, write **`03_recon_tolerances.json`**, the machine form every `dbx-recon run|estimate --tolerances` call reads: `{"version": "<the .md version id>", "full_diff_row_threshold": <rows>, "sample_size": <rows>, "numeric_abs_tol": <abs>, "aggregate_rel_tol": <rel>, "source_concurrency": <legacy query cap>}`, each value copied from a row of the `.md` and `version` equal to the record's current version. A tolerance amendment re-issues both files together, and `result.json` records the version it ran under, so a run against a stale JSON is visible in evidence.
4. Initialize `04_dependency_register.md` (empty, with the D1-D10 taxonomy header). `05_progress.md` is not written by hand: it is regenerated from `waves/*.result.json` with `python3 <plugin>/skills/migration-fanout/progress.py .migration` at every wave close and read as the cutover-readiness view; `06_decisions.md` rows carry **provenance and blast radius** (which unit forced the decision, which PR argued it, and which already-merged units' assumptions it invalidates), because a decision added at wave 3 that silently changes a wave-1 unit is otherwise invisible.
5. **Probe the environment and build `07_access_checklist.md`**: attempt a read query against the legacy system, a query on the Databricks warehouse, and a write to the target catalog. The probe must cover the **promotion schema** the target profile names, not only the catalog: pass `--analytical-schema <catalog>.<schema>` to the doctor in step 7; if that schema does not exist yet, create it now (`databricks schemas create <schema> <catalog>`) so the migration principal owns it and no grant is ever needed; if it exists under another owner, the red row is a D10 whose fired request is the row's `detail` (ready-to-paste GRANT statements) plus the closing reply `granted <D10 id>`. Record each as WORKS / BLOCKED with evidence. For every BLOCKED item, append a D10 entry to the dependency register and fire the concrete request (named approver, exact permission, suggested command or console path) via the user.
...
7. Write `.migration/allowed_targets.json` during setup and commit it before STOP A; if a front door already wrote it, complete it in place rather than recreating it. It is read by both the `dbx-recon` CLI and the plugin's PreToolUse guard (`hooks/dbx_guard.py`), so it carries the whole write-scope contract: `"catalogs"` (only the designated migration catalog), `"legacy_sources"` (the legacy secret names, hosts, DSNs, and profile names from `07_access_checklist.md`; any non-read statement mentioning one is blocked), `"guard_mode"` (`block`, the default; `warn` only by a recorded decision in `06_decisions.md`, for a false-positive command shape the engagement cannot rewrite), `"target_hosts"` (the fixture hosts and Lakebase DSN names a generic SQL client such as `psql`/`sqlcmd` may write to; the write must still resolve to a `"catalogs"` entry -- three-part name or the `-d`/URI database -- so the Lakebase database is named there too; the guard fails closed, so leave it empty and every such write blocks), `"bundle_targets"` (the `-t/--target` names `databricks bundle deploy|run|destroy` and `dbt run|build|seed` may use; empty means every deploy blocks; `prod`/`production` stay denied through `forbidden_bundle_targets`), `"lakebase_projects"` (the Lakebase Autoscaling Postgres project ids a resource write may target; empty means every Lakebase write blocks) and `"lakebase_branches"` (optional `fnmatch` globs for branch ids; empty allows any non-`production` branch). Example:

   Optional `"run_mode"` is `"live"` by default or `"fixture"` to require declared local cloud endpoints.
   Optional `"fixture_endpoints"` lists endpoint environment variables such as `"AWS_ENDPOINT_URL"`.

   ```json
   {"catalogs": ["mig_cat"], "legacy_sources": ["LEGACY_TD_DSN", "tdprod.corp"], "guard_mode": "block",
    "target_hosts": ["LAKEBASE_MIGRATION_DSN"], "bundle_targets": ["migration"],
    "lakebase_projects": ["loan-servicing-mig"], "lakebase_branches": ["mig-*"]}
   ```

   Adding a catalog, host or target later is a decision, never an edit. Add `.migration/.hook_probe_nonce` and `.migration/live_playbooks.json` to `.gitignore`. If `.migration/playbooks.lock.json` does not exist in this repo yet (the library is org-scoped, the lock is per engagement), run the `install-dbx-factory` skill here now: with the library already matching the repo it makes no create/update call, only verifies the 14 live bodies and writes the lock; commit it. Then run the `factory-doctor` preflight with `--role setup --source-secret <the legacy DSN's env var name>` (`--role setup` makes a still-missing `.migration/playbooks.lock.json` a warning instead of a red row, so an install run that stopped on an approval does not block setup — but the planning and orchestrator doctor runs need the lock, so resolve it before STOP C; that lock is install-dbx-factory's receipt, written only after its live re-read of all 14 playbooks passes — a failed or halted sync leaves no lock, so a red `playbooks_in_sync` row means re-run install-dbx-factory, never hand-write the lock; analytical track / warehouse front door: always pass `--analytical-schema <catalog>.<schema>`; OLTP track: pass it for the analytical track's schema and also pass the `--lakebase-*` flags, see factory-doctor) (the doctor proves that principal cannot write: `source_principal_read_only`; a DSN that can write is a D10 for a SELECT-only login, never an argument that `readonly=True` is enough) and commit its `09_capabilities.json`; a red row on identity, harness, hook loading, source principal, analytical target grants, or an uncommitted allowlist/tolerance file (`allowlist_committed`) is a D10, not something to work around.
