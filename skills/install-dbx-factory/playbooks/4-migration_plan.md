...
## Procedure
1. Run factory-doctor with the hook probe, expected catalogs, source secret/parameters, and all unit mappings. Refresh `09_capabilities.json`; `fail`, `unverified` hook state, or an uncommitted allowlist/tolerance is a D10 blocker. A red `named_secrets_exist` row (a `scope/key` a brief references that `databricks secrets list-secrets` does not list) is a STOP C blocker: create the named secret before STOP C, never leave it to a child to discover.
...
5. Write each `.migration/waves/wave-<N>.json` with repo, child/verifier macros, width, breaker, `auto_merge`, source family/secret/params, batch ids/units/write targets/briefs, each batch's `lakeflow_pipelines` (`[]` when it updates none) plus `serialized_pipelines` when two batches share one (rule in `skills/target-routing/SKILL.md`), capability contract, `verify_depth`, `max_minutes`, `cost_estimate`, and executable `gates`; commit manifests before STOP C. Sibling pipelines name manifests `wave-<pipeline>-<N>.json` (parallel-pipelines rule in `9-orchestrator.md`).
   Where the dialect skill emits a dependency analysis for a unit, take `write_targets` from its transitive writes (`skills/target-routing/SKILL.md`, "Route by call graph"); the workflow refuses a list that differs.
