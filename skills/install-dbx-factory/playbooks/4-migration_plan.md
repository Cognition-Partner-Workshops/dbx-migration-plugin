...
5. Write each `.migration/waves/wave-<N>.json` with repo, child/verifier macros, width, breaker, `auto_merge`, source family/secret/params, batch ids/units/write targets/briefs, capability contract, `verify_depth`, `max_minutes`, `cost_estimate`, and executable `gates`; commit manifests before STOP C. Sibling pipelines name manifests `wave-<pipeline>-<N>.json` (parallel-pipelines rule in `9-orchestrator.md`).
