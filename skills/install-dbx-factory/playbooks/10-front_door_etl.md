Playbook: Front door for ETL-tool estates (Informatica, Talend, Ab Initio, DataStage, SSIS). Thin intake that pins the source family, loads the right dialect skill, sets family-specific defaults, and then runs `!dbx_migrate_pipeline` in the same session. No migration method lives here.
...
2. **Create the write-scope allowlist before touching any source system**: `mkdir -p .migration` and write `.migration/allowed_targets.json` now, with `"catalogs"` set to the designated migration catalog from intake and `"legacy_sources"` filled with every legacy host, DSN and secret name the intake names (the setup playbook completes the other fields later). No source change of any kind — not a metadata probe with side effects, not a customer-approved DDL prerequisite — runs before this file exists; the guard is a no-op without it, and an authorized legacy write is then recorded only by hand. A source change the customer authorizes (a CDC prerequisite, supplemental logging) carries `--decision D-<id>` (or `DBX_DECISION=D-<id>`) pointing at a `06_decisions.md` row containing `legacy_write_authorized` and the object name; see the guard section of the README. No source change of any kind runs before the allowlist exists.
3. Confirm tool, version, and export mechanism; obtain or request the export (register D10 if blocked).
4. Attach the matching source-dialect skill: list `skills/*/SKILL.md` in the installed plugin and pick by name (`informatica-xml` today; `tsql-ssis` covers SSIS packages as it lands; Ab Initio, Talend, DataStage have no skill yet). If none exists for this tool, say so: the inventory can still run generically, but conversion quality depends on the skill, so building it from `skills/_dialect-skill-template.md` becomes wave-0 work, presented as such. Load `target-routing` alongside; it names the official Databricks skill (`databricks-pipelines`, `databricks-jobs`) for each conversion target. Even where a skill exists, **budget extractor hardening as engineering work on the first engagement per dialect**: lineage extraction from parameter indirection and dynamic components is the kit's highest-consequence failure point, and the skill's parser earns trust by surviving one real export, not by existing.
5. Set family defaults for the chain: unit = mapping/job + its session/workflow config; lineage extraction = parser-based from the export (not query history); PIPELINE profile is the dominant workload surface; scheduler edges are first-class D5 entries; parameter-file indirection is a named inventory risk.
