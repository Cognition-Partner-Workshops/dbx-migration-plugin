...
4. Attach the matching source-dialect skill: core ships `oracle-plsql`; `teradata-bteq`, `informatica-xml`, and `tsql-ssis` are optional skills under `skills-extra/` (install per its README before intake). No Redshift skill exists; Redshift follows the generic-ANSI plus build-the-skill path. If none exists for this tool, say so: the inventory can still run generically, but conversion quality depends on the skill, so building it from `skills/_dialect-skill-template.md` becomes wave-0 work, presented as such. Never claim a skill exists because an adapter or Lakebridge flag exists. Load `target-routing` alongside; it names the official Databricks skill (`databricks-pipelines`, `databricks-jobs`) for each conversion target. Even where a skill exists, **budget extractor hardening as engineering work on the first engagement per dialect**: lineage extraction from parameter indirection and dynamic components is the kit's highest-consequence failure point, and the skill's parser earns trust by surviving one real export, not by existing.
