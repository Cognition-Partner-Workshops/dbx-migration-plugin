...
6. **Waves 2..N, full fan-out, with the circuit breaker armed.** One wave = one launch, one gather, one gate, one brief. You do not review children one at a time.
   ...
   - **On a halt** (collision or breaker): one notification with the failure class, the batches affected, and the recommended fix. Patch the dialect skill or knowledge note, re-run the doctor with `--wave` for a fresh signature, update the pointer to `mode: "resume"` with the same recorded `run_id`, and relaunch. Use `mode: "rerun"` only for a deliberate redo. Do not let a new systematic trap cost 20x what it should cost once.
