...
6. **Waves 2..N, full fan-out, with the circuit breaker armed.** Per wave: launch one child per batch in parallel at the confirmed width, each with its complete hand-off from the plan (children share no filesystem or memory with you). While children are in flight, monitor for two halt conditions: (a) **write-target collision**: any child reporting a ledger collision halts further launches until the lineage gap is diagnosed and the batch table repaired; (b) **systematic failure**: when the circuit-breaker threshold from the plan (default 3 children) report the same failure class, pause remaining launches, patch the dialect skill or knowledge note, relaunch the failed children, then resume. Do not let a new systematic trap cost 20x what it should cost once. As children land: verify recon evidence per PR, then run the wave's **single live recon window**: children iterated fixture-first, so the authoritative live proof is one uncontended parent-side pass via `!dbx_data_reconciliation` over the completed wave, merge green PRs together, fire **STOP D** with the wave's PRs and recon report in batch. Reopen failed units as fresh children with the failure evidence attached. Never MERGE wave N+1 before wave N is merged and green, but pipeline the phases: once wave N's children have landed and registered targets, wave N+1's conversion children may launch while wave N's independent recon and review run concurrently (they touch disjoint resources: recon reads wave N's targets, conversion writes isolated areas). At each wave close, append the wave's recon report, PR list, and exceptions to the evidence-pack skeleton in `.migration/`, so cutover signoff verifies a pack instead of assembling one, and write the **wave-close brief**: one page for a 5-minute read covering what landed, what was decided and why, what broke in review and what that implies for later waves, what remains unproven, and the per-unit cost line (session ACUs, warehouse-hours) recorded now, not reconstructed later; the cost line is what makes a runaway re-run loop visible while it is still cheap to stop. The brief accompanies the STOP D notification; a lead's whole day should be readable in it plus the one-screen status table in `05_progress.md`. Within a wave, apply the plan's calibration rule: one small unit of any new pattern class first, harvest its feedback, then the rest of the class in parallel; do not serialize a whole wave to one child per wave when its units are independent.
   For large estates, drive this step as a **dynamic workflow** (deterministic fan-out script with structured outputs and journal-based resume) rather than hand-managing dozens of sessions; the workflow enforces the same gates mechanically, and it is also the cost-correct choice: the orchestrator agent spends compute only on exceptions and wave close, not on the launch/wait/collect loop.
...
- Do NOT assume a child succeeded; read its PR and recon evidence.
