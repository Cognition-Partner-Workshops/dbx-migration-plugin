...
`stop_mode` also decides merge authority. Every wave manifest sets `auto_merge` explicitly from the effective mode: `true` in soft mode, `false` in hard mode (the workflow treats a missing value as `true`, so never omit it). The same rule covers every PR the orchestrator owns outside a manifest (wave 0, wave-close, coexistence, cutover documents) and the small-wave path: soft merges PASS PRs as they are verified; hard holds them and merges only after the batched wave-close reply (one prompt per wave, never per PR). Independently of mode, when the workflow itself disables merging after a write-target anomaly (overlap, undeclared target, missing target report) the affected PRs stay unmerged until a human resolves the halt; a soft default never clears a safety halt. A child's ESCALATE that carries a recommended answer is resolved with it after the 60-second window in soft mode (same provenance row) and waits for a reply in hard mode.
