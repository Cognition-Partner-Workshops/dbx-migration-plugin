...
| Shared tables across waves | Reads every `wave-*.json` in `.migration/waves/` (one that is not JSON or has no `batches` rows halts). A target written by units in different waves is shared: a later wave's rows would undo the earlier unit's whole-table recon, so every `mapping_spec.json` object that reads it must carry a `target_where` bounded to that unit's own partition or run date: it must parse under the harness predicate grammar and, at every parenthesis depth, every `OR` branch must compare a column of the table while an `AND` needs one such operand (`1 = 1`, `TRUE`, literal-only or parameter-only comparisons, and `(col = x OR 1 = 1)` are no bound). Target names compare case-insensitively with quoting and whitespace stripped, and a bare mapping object (`t`) reads the qualified target (`mig.t`); `mig.t` and `other.t` stay distinct. An unbounded reader, or a current-wave unit with no mapping for the table, halts before launch naming the table and both batches. |
