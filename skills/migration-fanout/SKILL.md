...
| Merge authority | A child's PASS is recorded only when it reports `merge_eligible: true` and every unit's own `.migration/recon/<unit>/result.json` at the gated PR head says `merge_eligible: true` (the batch reports one boolean for up to five units; the per-unit files decide), or with `merge_authority: {kind: human_override, decision_id: D-<n>}` where the row of `.migration/06_decisions.md` whose id cell is that `D-<n>` (a row that only cites it in prose is another decision) carries human provenance (`user:<id>`, the reply's message or event id), names every unit of the batch and says `merge_override`. Anything else (a unit file missing or without a boolean `merge_eligible`, a `harness` claim on `false`, an id the ledger lacks, a `default-accepted` row, a row naming other units) becomes FAIL with `failure_class: merge_authority`. The result's `merge_overrides` and the brief list every override with its decision id. |
| Acceptance gates | Each batch declares `gates: [{id, kind: byte_compare\|export_file\|publish_leg\|row_parity\|structural\|custom, status: pending\|passed\|failed\|waived, evidence, decision_id?}]`, the gates STOP C approved for its units, and the manifest carries `gates_sha` = sha256 of the sorted `{batch id: [[sorted unit ids], [[gate id, kind], ...]]}` map as compact JSON. The STOP C approval is a `D-<n>` row of `.migration/06_decisions.md` with `user:<event-id>` provenance that names `STOP C` and says `gates_sha <value>`; the workflow halts before launch unless the manifest's value is in such a row (the manifest cannot approve its own list), and again if the declared list no longer hashes to it (a gate renamed, added, dropped or swapped for another kind after STOP C); the error prints the hash the current list has. A child reports `gates: [{id, status: passed\|failed, evidence}]`: `passed` needs non-empty evidence and it cannot waive, rename or re-kind a gate or change one the plan already recorded `passed`/`waived` (repeating a `passed` gate with the recorded evidence is accepted; the prompt lists only `pending`/`failed` gates). A PASS with any gate not `passed`, or `waived` without a `D-<n>` row of `.migration/06_decisions.md` that says `waive` and names the gate id and every unit of the batch, becomes FAIL with `failure_class: gates`, so the wave cannot close over it. The result's `waived_gates` and the brief list every waiver with its decision id. A small wave gathered by hand applies the same rule with `python3 skills/migration-fanout/workflow.py gates <results.json>` (the children's `[{batch, gates}]` rows; exits non-zero on any unmet gate). |
