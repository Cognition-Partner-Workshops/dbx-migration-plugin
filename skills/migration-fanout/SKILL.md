...
| Acceptance gates | Each batch declares `gates: [{id, kind: byte_compare\|export_file\|publish_leg\|row_parity\|structural\|custom, status: pending\|passed\|failed\|waived, evidence, decision_id?}]`, the gates STOP C approved for its units, and the manifest carries `gates_sha` = sha256 of the sorted `{batch id: [[gate id, kind], ...]}` map as compact JSON. Any missing or malformed field, or a `gates_sha` that no longer matches (a gate renamed, added, dropped or swapped for another kind after STOP C), halts before launch; the error prints the hash the current list has. A child reports `gates: [{id, status: passed\|failed, evidence}]`: `passed` needs non-empty evidence and it cannot waive, rename or re-kind a gate or change one the plan already recorded `passed`/`waived`. A PASS with any gate not `passed`, or `waived` without a `D-<n>` row of `.migration/06_decisions.md` that says `waive` and names the gate id and every unit of the batch, becomes FAIL with `failure_class: gates`, so the wave cannot close over it. The result's `waived_gates` and the brief list every waiver with its decision id. |
