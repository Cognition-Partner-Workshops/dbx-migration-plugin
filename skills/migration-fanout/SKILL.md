...
| Merge authority | A child's PASS is recorded only with `merge_eligible: true` copied from `result.json`, or with `merge_authority: {kind: human_override, decision_id: D-<n>}` where that row of `.migration/06_decisions.md` names every unit of the batch and says `merge_override`. Anything else (missing or non-boolean `merge_eligible`, a `harness` claim on `false`, an id the ledger lacks, a row naming other units) becomes FAIL with `failure_class: merge_authority`. The result's `merge_overrides` and the brief list every override with its decision id. |
