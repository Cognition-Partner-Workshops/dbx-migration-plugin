...
def test_render_progress_uses_manifest_units_when_result_omits_them(tmp_path):
    ...
    assert "| 1 | w1-b01 | u_a | PASS (unmerged) | PASS |  | https://example.invalid/1 | pending | {\"rows\":2} |" in text
    assert "| 1 | w1-b01 | u_b | PASS (unmerged) | PASS |  | https://example.invalid/1 | pending |  |" in text
...
def test_render_progress_sorts_rows_and_lists_wave_status(tmp_path):
    ...
    assert "| 1 | a | u2 | PASS (unmerged) | PASS |  | https://example.invalid/2 | pending |  |" in lines
    assert '| 2 | b | u1 | PASS | PASS |  |  | pending | {"rows":2} |' in lines
    assert '| 2 | b | u2 | PASS | PASS |  |  | pending |  |' in lines
    assert lines.index("| 1 | a | u2 | PASS (unmerged) | PASS |  | https://example.invalid/2 | pending |  |") < \
...
def test_render_progress_ignores_stale_merged_record_head(tmp_path):
    ...
    assert "| 1 | b1 | u1 | PASS (unmerged) |  |  | https://example.invalid/1 | pending |  |" in render_progress(mig)
