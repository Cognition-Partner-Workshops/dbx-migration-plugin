...
def test_render_progress_uses_manifest_units_when_result_omits_them(tmp_path):
    ...
    assert "| 1 | w1-b01 | u_b | PASS | PASS |  | https://example.invalid/1 | pending | {\"rows\":2} |" in text
...
def test_render_progress_sorts_rows_and_lists_wave_status(tmp_path):
    ...
    assert lines[1] == "| wave | batch | unit | status | recon_verdict | verifier_verdict | pr_url | merged | recon_cost |"
...
def test_render_progress_empty_waves_has_header_and_table(tmp_path):
    ...
    assert text.splitlines() == [
        ...
        "| wave | batch | unit | status | recon_verdict | verifier_verdict | pr_url | merged | recon_cost |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
