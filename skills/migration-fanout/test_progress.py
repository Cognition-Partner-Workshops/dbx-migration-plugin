...
def test_render_progress_sorts_rows_and_lists_wave_status(tmp_path):
    ...
    assert '| 2 | b | u1 | PASS | PASS | PASS |  | pending | {"rows":2} |' in lines
    assert '| 2 | b | u2 | PASS | PASS | PASS |  | pending |  |' in lines
    assert lines.index("| 1 | a | u2 | PASS (unmerged) | PASS | PASS | https://example.invalid/2 | pending |  |") < \
        lines.index("| 1 | z | u3 | FAIL | FAIL | FAIL |  |  |  |")
