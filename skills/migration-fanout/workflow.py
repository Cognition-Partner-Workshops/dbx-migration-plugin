...
def capability_block():
    ...
    return (
        ...
        f"--expect-identity {caps['identity']}, one --mapping per unit in this batch "
        "(.migration/units/<unit_id>/mapping_spec.json), --source-secret naming the secret your recon "
        "gate passes as --source-dsn-secret, and the same --param values the gate will get; then "
        "complete its hook probe. Any 'fail' row (identity mismatch, harness missing, hooks not "
        "applied, allowlist differs from the contract, unit mappings omitted, declared delete "
        "evidence not readable on the source) "
