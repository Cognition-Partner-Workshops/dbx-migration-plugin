...
| `BEGIN TRAN ... COMMIT / ROLLBACK`, `SAVE TRAN`, `@@TRANCOUNT` nesting | transaction spanning one table -> single-statement atomicity (`MERGE`/`INSERT ... SELECT`), no transaction needed. Transaction spanning > 1 table -> **compensating `EXIT HANDLER`**: every write keyed by a batch/run key, the handler deletes them in reverse order and restores updated columns from a temp-table snapshot taken before the first write (`examples/proc-cursor-payments`) | `BEGIN ATOMIC ... END` around the writes when every table involved was created with `delta.feature.catalogManaged` (Preview) | Python connector `autocommit=False` | `[dbsql:scripting]` "Key rules" (`NOT ATOMIC` is the default and only compound-statement behavior: no automatic rollback), "Multi-Statement Transactions", "SQL Scripting Atomic Blocks", "Python Connector Transaction API". Never convert a multi-table transaction to a bare sequence of DML: a failure between statements leaves a state the source could never produce (payments without balance updates) and no recon tier on a clean run detects it, only the failure-injection replay does |
...
| **Multi-table transaction flattened to sequential DML** | per-row `BEGIN TRAN ... COMMIT` keeps payment + balance update together | compound statement is `NOT ATOMIC` `[dbsql:scripting]`: a failure after write k leaves writes 1..k committed | none on a clean run (all tiers pass); failure-injection replay shows N payments with unchanged balances | compensating `EXIT HANDLER` (§6 transactions row) or `BEGIN ATOMIC` on `catalogManaged` tables | decision: compensation vs Preview atomic block, recorded per unit |
...
| Transactions | explicit `BEGIN TRAN` spanning > 1 table; `SAVE TRAN`; `@@TRANCOUNT` checks | +1; cross-table atomicity required: +2 (compensating `EXIT HANDLER` with snapshot restore, or Preview `BEGIN ATOMIC` on `catalogManaged` tables; §7 "Multi-table transaction flattened") |
