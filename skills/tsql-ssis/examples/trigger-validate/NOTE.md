...
- `RAISERROR 50050 '...'` + `ROLLBACK TRANSACTION` -> `DECLARE ... CONDITION FOR SQLSTATE '45050'` + `SIGNAL` **before** the DML, both **inside the writer's `BEGIN ATOMIC ... END` block** (`databricks-dbsql` references/sql-scripting.md "Exception Handling", "SIGNAL", "SQL Scripting Atomic Blocks"; docs `/aws/en/transactions/transaction-modes` shows `IF ... SIGNAL ... END IF` guarding a `MERGE` inside one block). The block's snapshot read makes check and DML see the same rows, so a concurrent balance change cannot slip between them: it is either in the snapshot or fails the commit as a write-write conflict (docs `/aws/en/transactions/` "Conflict detection and concurrency"). `SIGNAL` rolls the block back including the converted `BALANCE_VIOLATION` row, so the end state matches ASE (which lost its trigger's audit row inside the caller's rolled-back transaction). A writer that runs the check outside a transaction both commits the row (tolerance record: `>=` source) and races the DML; recorded, not recommended.
...
- **Tier 1** on `audit_trail` grouped by `action_type`: a writer that forgot to fold `trg_audit_payment` has zero `PAYMENT_INS` rows for its batch; `BALANCE_VIOLATION` counts equal the source for writers that run the check inside their atomic block, and are >= source only for a writer that runs it outside one (documented per writer).
