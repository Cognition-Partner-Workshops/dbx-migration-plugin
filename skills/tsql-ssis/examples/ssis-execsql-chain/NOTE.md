...
- **Expression constraint** (`EvalOp=3` ExpressionAndConstraint, `@[User::EligibleCount] > 0`) -> `IF` guard inside the consuming task's SQL script (`[dbsql:scripting]`) because the `SingleRow` -> variable binding has no SQL-task equivalent; the graph-visible alternative (notebook task value + If/else condition task) is named with its citation (`[jobs:taskvalues]`, `[jobs:ifelse]`).
...
- **OnError event handler** writing `dbo.audit_trail` with `System::SourceName`/`System::ErrorDescription` -> folded into the `log_failure` task (`{{job.run_id}}` recorded instead; system variables have no SQL equivalent) plus `email_notifications.on_failure` (`[jobs:monitor]`).
