...
Delta appended with `skills/tsql-ssis/SKILL.md` §10 (SEEDED against the Sybase ASE fixture `ts-tsql-sybase-legacy-db` and the three built `.dtsx` descriptions; no engagement unit ids yet). Additive to the `mssql`/`ssis` rows above, not a rewrite:
