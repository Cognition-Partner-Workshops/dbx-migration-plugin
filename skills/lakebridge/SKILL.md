...
### `oracle` delta (SEEDED, 2026-09-08, from `skills/oracle-plsql` fixture round-trip; no engagement unit yet)
Additions to the `oracle` row above for `--source-dialect oracle`; the base row is unchanged. Recon signatures name the `oracle-plsql` SKILL.md Known Trap (§7) that owns them; fixture object in brackets.

| Class | Construct | Expectation and recon signature |
|---|---|---|
| Converts | `LISTAGG ... WITHIN GROUP`, `NVL2`, `TO_CHAR(d, fmt)` with `YYYY/MM/DD/HH24/MI/SS` only, `ADD_MONTHS`/`LAST_DAY`, `MERGE ... WHEN MATCHED ... DELETE WHERE` | trust after a read; `DELETE WHERE` must come out as a separate `WHEN MATCHED AND <cond> THEN DELETE` ordered before the `UPDATE` (§7 trap 8) [`09_mrg_policy_from_stg`] |
| Mangles silently | `CHAR(n)` compares against `VARCHAR2` (`postcode = 'SW1A 1AA'`) | blank-padded semantics lost; Tier 3 mismatches on `CHAR` keys, Tier 1 undercount on joins; canonicalize `rstrip_spaces` and `rtrim()` the converted predicate (§7 trap 5) [`02_tbl_party_broker`, `14_rpt_policy_page`] |
| Mangles silently | `ROWNUM` sandwich pagination (`WHERE ROWNUM <= hi` outer, `rn > lo` inner) | rewritten to `LIMIT` without the inner `ORDER BY` being preserved as the total key; Tier 4 page contents differ, Tier 1 per-page count is right (§7 trap 7) [`14_rpt_policy_page`] |
| Mangles silently | `TO_CHAR(n, 'FM...')`, `TO_CHAR(d, 'DD-MON-YYYY')` | `FM` dropped (leading blanks) and `MON` lower-cased (`Jan` vs `JAN`); Tier 4 byte diffs on report columns (§5 #44, #45) [`14_rpt_policy_page`] |
| Mangles silently | `TRUNC(d, 'MM')` / `TRUNC(d, 'IW')` in MV group keys | `'MM'` to `date_trunc('MONTH')` fine; `'IW'` (ISO week) and `'Q'` silently become day truncation or fail typing; Tier 1 group counts (§5 #61) [`12_mv_policy_premium_summary`] |
| Mangles silently | `a || b` with a nullable operand | `concat()` returns NULL where Oracle returns the non-null side; Tier 3 null-vs-value diffs on derived name/address strings (§7 trap 23) [`14_rpt_policy_page`] |
| Mangles silently | `CONNECT BY NOCYCLE ... CONNECT_BY_ISCYCLE` | recursive CTE without a cycle guard loops to the recursion limit or drops the cycle row Oracle emits; Tier 1 row count on hierarchy views (§7 trap 6) [`10_qry_broker_hierarchy`] |
| Mangles silently | `SUM(NUMBER)` over an all-NULL group in an MV | Oracle NULL vs pipeline MV 0; Tier 2 with `null_missing_equiv` **off** on the aggregate columns (§7 trap 25) [`12_mv_policy_premium_summary`] |
| Rejects / hand-convert | `CREATE MATERIALIZED VIEW LOG`, `REFRESH FAST`, `ENABLE QUERY REWRITE`, `DBMS_MVIEW.REFRESH` | Lakeflow Pipelines MV by hand (`oracle-plsql` example 05); MV logs are absorbed, rewrite has no equivalent (§7 trap 15) |
| Rejects / hand-convert | `DBMS_SCHEDULER.CREATE_JOB/CREATE_PROGRAM`, calendar strings, `ADD_JOB_EMAIL_NOTIFICATION` | Lakeflow Jobs DABs by hand; calendar string to quartz cron + `timezone_id` (§7 trap 16) |
| Rejects / hand-convert | SQL*Plus directives (`SET`, `DEFINE`/`&n`, `WHENEVER`, `SPOOL`, `EXIT`) | strip and re-express as job parameters / task outcome (§7 trap 17); a transpiler run on a `.sql` with these produces parse errors for the whole file, so split the SQL body out first |
| Rejects / hand-convert | `CREATE OR REPLACE TRIGGER` with `:NEW`/`:OLD`, `PRAGMA AUTONOMOUS_TRANSACTION`, `FOR UPDATE SKIP LOCKED`, `SAVEPOINT`/`ROLLBACK TO` | OLTP-profile units go to Lakebase/Postgres by hand (example 04); analytical units fold trigger logic into the writer (§7 traps 10, 14) |
| Rejects / hand-convert | `CREATE SYNONYM`, `@dblink` references, `CREATE DATABASE LINK`, VPD (`DBMS_RLS`) / `DBMS_REDACT` policies, `GRANT ... TO PUBLIC` | inventory only; resolve synonyms in lineage, external links stay `INFERRED` edges, policies map through `databricks-unity-catalog` or `GAP` (§7 traps 18, 19) [`15_syn_dblink_grants`] |
