...
BEGIN
  ...
  CREATE OR REPLACE TEMPORARY VIEW exp_claim_dates AS
  SELECT claim_id,
  ...
  CREATE OR REPLACE TEMPORARY VIEW exp_claim_flags_status AS
  SELECT claim_id,
  ...
  SELECT u.*, 'TO_DATE row error' AS reject_reason, current_timestamp() AS rejected_at
  FROM   un_claim_sources u JOIN exp_claim_dates d USING (claim_id)
  WHERE  d.loss_dt_row_error;
  ...
  SELECT u.claim_id, u.in_SOURCE_SYSTEM, d.out_LOSS_DT, f.out_FRAUD_FLAG, f.out_CLAIM_STATUS, u.event_ts,
         current_timestamp()
  FROM   un_claim_sources u
  JOIN   exp_claim_dates d USING (claim_id)
  JOIN   exp_claim_flags_status f USING (claim_id)
  WHERE  NOT d.loss_dt_row_error;

  SET p_rows_loaded = (SELECT COUNT(*) FROM un_claim_sources u JOIN exp_claim_dates d USING (claim_id)
                       WHERE NOT d.loss_dt_row_error);
END;
