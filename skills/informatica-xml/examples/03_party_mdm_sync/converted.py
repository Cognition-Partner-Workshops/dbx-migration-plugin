...
# as the pure functions below (wave-0 library) and imported by both units. Sources/targets are not in the export;
# .par: $DBConnection_SRC=TD_POLICY_ADMIN_PROD, $DBConnection_APF=TD_CORE_BANKING_PROD, $DBConnection_TGT=ORA_MDM_HUB_PROD.
...
def mdm_party_golden():
    ...
    # EXP_EMAIL_DQ: LOWER(LTRIM(RTRIM(x))) -> lower(trim(x)); REG_MATCH -> rlike (Perl-style, portable to Java)
    ...
    cust = spark.read.table(CUSTOMERS_TABLE).select(
        F.col("CUSTOMER_ID").alias("LEGACY_CUSTOMER_ID"), F.col("NINO").alias("cust_nino"),
    ...
    nino_match = one_per_party(xm.join(cust, xm.NINO == cust.cust_nino), "nino_customer_id")
    fuzzy = one_per_party(
        xm.join(cust, (xm.SURNAME_SOUNDEX == cust.cust_soundex) & (F.to_date(xm.BIRTH_DT) == cust.cust_dob)),
        "fuzzy_customer_id")

    return (party.join(xm.drop("NINO"), "PARTY_ID", "left").join(nino_match, "PARTY_ID", "left").join(fuzzy, "PARTY_ID", "left")
                 .select("PARTY_ID",
                         mplt_name_std(F.col("FIRST_NAME")).alias("FIRST_NAME_STD"),
                         mplt_name_std(F.col("LAST_NAME")).alias("LAST_NAME_STD"),
                         mplt_phone_std(F.col("PHONE")).alias("PHONE_STD"),
                         F.nullif(email_std, F.lit("")).alias("EMAIL_STD"),  # Oracle target stores '' as NULL (trap 5)
                         email_dq.alias("EMAIL_DQ_STATUS"), nino_masked.alias("NINO_MASKED"),
                         "SURNAME_SOUNDEX", "BIRTH_DT",
