...
# $$RUNDATE was rewritten into the .par per run, so it is read from the arrived file name; the config key is only
# an explicit-rerun override. $InputFile_* / $DBConnection_LKP become a volume path and a catalog.schema table.
...
LANDING = spark.conf.get("informatica.landing_path")
...
def plcymstr_raw():
    return (spark.readStream.format("cloudFiles")
                 ...
                 .option("encoding", "windows-1252")  # FLATFILE CODEPAGE=MS1252
...
def stg_policy_master():
    ...
    # Expressions are row-preserving: derive on the same row, never split into views re-joined on a non-unique key
