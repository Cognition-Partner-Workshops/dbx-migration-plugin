...
| Three row-preserving Expressions fanning out from one SQ row and back into one target row: converted as column derivations over the same `sq_plcymstr_daily` row (`exp_policy_dates`/`exp_postcode_dq`/`exp_policy_flags` helpers), never as views re-joined on `PLCY_POLICY_NO` (no declared unique key in the flat file; a rejoin multiplies repeated policy numbers) | `SQ -> EXP_* -> STG_POLICY_MASTER` connectors | rows 69-70; Tier 1 row count catches a rejoin |
