...
`canonicalization.json`'s `type_map` marks `bigint` for `INTEGER`/`INT`/`SMALLINT` and `date` for `DATE` as conditional alternatives: the spec's field must carry rule `census_fits_int64` (census proved the values fit in 64 bits) or `census_midnight_only` (census proved `TRUNC(col)=col` on every row) or the declared type is a contradiction.
...
| `LISTAGG(x,',') WITHIN GROUP (ORDER BY k)`, `WM_CONCAT` | `listagg(x,',') WITHIN GROUP (ORDER BY k NULLS LAST)` / `string_agg` (DBR 16.4+; else `array_join(transform(array_sort(collect_list(struct(k IS NULL, k, pk, x))), s -> s.x), ',')`: sort on the `WITHIN GROUP` key plus a tie-breaker, never on `x`; the leading `k IS NULL` puts NULL keys last) | edge | Oracle sorts NULL keys last (ASC) / first (DESC), Databricks the reverse: always spell `NULLS LAST/FIRST` (trap 22); both drop NULL `x`; `ON OVERFLOW TRUNCATE` has no clause; `WM_CONCAT` order was never defined |
