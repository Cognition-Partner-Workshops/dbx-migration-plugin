...
| `SYSDATE`, `SYSTIMESTAMP`, `CURRENT_DATE/TIMESTAMP`, `DBTIMEZONE` | `current_timestamp()` cast `TIMESTAMP_NTZ`, `current_timestamp()`, `current_date()`, `current_timezone()` (L: `localtimestamp`, `now()`) | edge | `SYSDATE` = server TZ, advances inside PL/SQL loops; `CURRENT_*` = session TZ; Databricks fixes per query |
...
| `LISTAGG(x,',') WITHIN GROUP`, `WM_CONCAT` | `listagg` / `string_agg` (DBR 16.4+; else `array_join(sort_array(collect_list(x)),',')`) | edge | `ON OVERFLOW TRUNCATE` has no clause; `WM_CONCAT` order was never defined |
...
| `MERGE ... WHEN MATCHED THEN UPDATE ... DELETE WHERE ... WHEN NOT MATCHED ... WHERE` | `MERGE` with `WHEN MATCHED AND cond THEN DELETE` **first**, `WHEN NOT MATCHED AND cond` `[docs:delta-merge-into]` | edge | `DELETE WHERE` sees post-update values in Oracle, pre-update in Delta; ORA-30926 parity via `assert_true` pre-check (trap 8, example 01) |
...
| 8 | `MERGE` duplicates, `DELETE WHERE`, `NEXTVAL` | ORA-30926 on any duplicate source key; `DELETE WHERE` sees post-update values; Delta raises only on duplicates hitting a target row, inserts unmatched duplicates twice, evaluates `WHEN MATCHED AND` on pre-update rows | Tier 1 count off by the deleted set / unmatched duplicates; Tier 4 target succeeded where Oracle failed (`QUALIFY` dedupe hides it) | `assert_true(count(*)=0)` over duplicate keys before the `MERGE`; `DELETE` clause first with a source predicate; exclude surrogate keys from Tier 3 |
