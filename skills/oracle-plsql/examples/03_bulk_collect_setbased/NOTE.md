...
| cursor `FOR UPDATE SKIP LOCKED` + `BULK COLLECT LIMIT` + `FORALL` (collections `DELETE`d per batch: one renewal per policy, also when the last batch is short) | candidate `TEMPORARY TABLE` (drop-first; a temp *view* is re-executed on every access) + one `MERGE` + `INSERT ... SELECT`; snapshot isolation inside `BEGIN ATOMIC` replaces row locks | construct map |
...
changes the count); Tier 2 `SUM(annual_premium)` per `product_cd` with `decimal_round` (double uplift, `DECIMAL`
...
**Canonicalization**: `decimal_round` (bare-`NUMBER` `annual_premium`, `amount`), `datetime_utc_truncate_ms` (`DATE` `expiry_dt`, `txn_dt`),
