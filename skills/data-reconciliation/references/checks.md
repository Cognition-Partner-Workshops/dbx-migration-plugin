...
`factory-doctor [--role child --unit <id> ...] --source-secret NAME [--param name=value ...]` (the
doctor resolves the unit mappings itself, every `--unit` of a child's batch or every
`.migration/units/*/mapping_spec.json` for an orchestrator, so no subset can pass as the whole; the
same `--param` values the run will get) verifies the source has it on, every declared capture exists and
...
on the `cdc` schema is required or checked). A red row is a customer decision to record (or drop
the block and drain), not a fix to apply.
