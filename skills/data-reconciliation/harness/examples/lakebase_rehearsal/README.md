...
zero findings, zero rows in flight. Cost 87 source statements / 6810 rows fetched, 55 target
...
adapter reset to `READ COMMITTED` and relied on the open/close markers, which held.
...
with all 500 rows in flight and *no* tier 3 findings for them; tier 3 `field_diff key=(5,)
current_balance` on `loans`. Tiers 1/2/5/7 clean.

## Not proven here

A real Lakebase branch (only the Postgres protocol and isolation level were exercised), a source
with snapshot isolation enabled (the fallback path ran instead), and a live CDC feed (lag was
injected, not observed from Lakeflow Connect).
