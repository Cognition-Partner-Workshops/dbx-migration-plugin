...
Delta appended with `../tsql-ssis/SKILL.md` (SEEDED 2026-09-08 against the Sybase ASE fixture `ts-tsql-sybase-legacy-db` and the `.dtsx`-shaped package in `../tsql-ssis/examples/ssis-dataflow-lookup/`; unit id: none, fixture seed, not an engagement unit). Additive to the `mssql`/`ssis` rows above, not a rewrite:
