...
| Analyzer inventory of the export (no transpiler flag exists) | embedded SQL overrides transpile under `teradata` / `oracle` | `../lakebridge/SKILL.md` Informatica row |
...
| 46 | `MD5(s)`, `CRC32(s)` | `md5(s)` / `crc32(s)`; hash `encode(s, 'ISO-8859-1')` under Latin1 | same hash family each; hash of non-ASCII and CHAR padding differ |
...
| 66 | Lookup dynamic cache (`NewLookupRow`, `Insert Else Update` / `Update Else Insert`, `Output Old Value On Update`) | stateful per input row, never a plain join or CDC flow alone: LEFT JOIN the pre-run target snapshot, then `row_number()` / `lag()` OVER (PARTITION BY lookup keys ORDER BY <INFERRED input order>); first occurrence: no snapshot row -> 1 (insert), values differ -> 2 (update), else 0; later occurrences compare to `lag()` of the previous input row (the cache now holds it); pre-image = snapshot row or `lag()` values; the policy flags decide which of 1/2 may write; persist the last row per key afterwards with `create_auto_cdc_flow` / `MERGE` | the cache mutates within the run, so repeated keys get 1 then 0/2 and later rows see earlier updates; input order is INFERRED (trap 9) |
...
- Mapping -> one SDP file: `@dp.table` Auto Loader read per file `SOURCE` definition (`_metadata.file_path` supplies `$$RUNDATE`-style values; a CSV schema applies by position, so layouts with different headers never share one stream), `@dp.temporary_view` per lookup deduplicated to one row per key, row-preserving Expressions as `withColumn` on the same row, `@dp.materialized_view` target named after the legacy target with port names as aliases, `@dp.expect*` for every legacy row-error/reject condition plus a quarantine MV built from the negated conditions.
...
| `examples/05_ri_bordereaux_monthly/` (`source.xml`, `converted.py`) | `RI_CESSIONS.m_RI_BORDEREAUX_MONTHLY` | one Auto Loader stream per broker `SOURCE` (`ISO-8859-1`, unioned by name), `CHR(163)`, `REPLACESTR` caseFlag 0 as `(?i)`, `TO_DECIMAL` 0-vs-NULL surfaced as an expectation, drifted reference Lookup deduplicated, unexpected-broker rows quarantined not published, warn-only expectations where the legacy had no Filter |
