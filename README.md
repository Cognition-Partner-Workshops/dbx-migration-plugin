...
{
  "requiredPlugins": [
    { "source": "github", "repo": "Cognition-Partner-Workshops/dbx-migration-plugin", "ref": "v0.2.1" }
  ]
}
```
...
**Authorized legacy writes.** A non-read statement naming a `legacy_sources` entry needs `--decision D-<id>` or
`DBX_DECISION=D-<id>` matching a `legacy_write_authorized` row in `.migration/06_decisions.md` that names every object
written. `guard_mode: warn` never downgrades an unauthorized legacy write.
